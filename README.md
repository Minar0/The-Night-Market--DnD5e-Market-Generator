# The Night Market
Ever wanted to generate a large market of merchants with a single button press? Try visiting The Night Market (TNM)

## Install
I'm assuming decent technical knowledge. Will probably make installation easier later

1. Make sure you have docker and docker compose installed 
2. Clone the repo
3. Run ```docker compose up``` from the repo's directory
4. Access it at http://127.0.0.1:8081/
5. Do fun webservery stuff. I have mine running 24/7 on my homelab

## Usage
Press the Generate Shops button to roll a bunch of shops. If you want to pass this smorgasburg of items to your players to peruse, press the Export Player HTML button

Then send your players the HTML and they should have access to the shops you just generated

If you'd like to modify the shops for your own purposes, every shop is an extension of a Merchant parent class. Feel free to modify the merchants available or add your own.
If you add any merchants, open a pull request and I'll probably add them to this repo

I'm hoping the code is relatively self-explanetory, but if not, let me know and I'll put together some actual documentation when I'm not dead tired

## Disclaimer
TNM is a weekend creation that I built in an obsession fueled blur and I've already forgotten how half this stuff works. 
Apologies if you try to modify it and it breaks immediatly
I've never actually tested it outside Firefox
